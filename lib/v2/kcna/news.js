...
const { fixDesc, fetchPhoto, fetchVideo } = require('./utils');
...
module.exports = async (ctx) => {
    ...
    const list = $('.article-link li a')
        .map((_, item) => {
            ...
            return {
                ...
                pubDate: timezone(parseDate(dateString.match(/\d+\.\d+\.\d+/)[0])),
            };
        })
        .get();
    ...
    for await (const item of asyncPool(3, list, (item) =>
        ctx.cache.tryGet(item.link, async () => {
            ...
            const dateString = dateElem.text();
            dateElem.remove();
            item.pubDate = parseDate(dateString.match(/\d+\.\d+\.\d+/)[0]) || item.pubDate;

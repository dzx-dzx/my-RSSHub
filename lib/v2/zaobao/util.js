    ...
    const resultList = await Promise.all(
        data.toArray().map((item) => {
            ...
            return ctx.cache.tryGet(link, async () => {
                let article;
                try {
                    article = await got_ins.get(link);
                } catch {
                    return {
                        link,
                        title,
                        // pubDate: timezone(parseRelativeDate($item.find("h2").text()), +8)
                    };
                }
                const $1 = cheerio.load(article.data);

                const title =
                    $1('#version-toggle').text().trim() === '新加坡版' || $1('.after\\:content-dropdown-triangle').text().trim() === '新加坡版'
                        ? JSON.parse($1("head script[type='application/ld+json']").eq(0).text())['@graph'][0]?.headline // SG
                        : JSON.parse($1("head script[type='application/ld+json']").eq(1).text()).headline; // HK
                const time = (() =>
                    $1('#version-toggle').text().trim() === '新加坡版' || $1('.after\\:content-dropdown-triangle').text().trim() === '新加坡版'
                        ? timezone(parseDate(JSON.parse($1("head script[type='application/ld+json']").eq(0).text())['@graph'][0]?.datePublished.replace(/(\d+\/\d+\/\d+) - (\d+:\d+)/, '$1 $2')), +8) // SG
                        : timezone(parseDate(JSON.parse($1("head script[type='application/ld+json']").eq(1).text()).datePublished), +8))(); // HK

                $1('.overlay-microtransaction').remove();
                $1('#video-freemium-player').remove();
                $1('script').remove();
                $1('.cx_paywall_placeholder').remove();
                $1('#imu').remove();

                let articleBodyNode = $1('.articleBody').length === 0 ? $1('.article-content-rawhtml') : $1('.articleBody');

    ...
    const resultList = await Promise.all(
        data.toArray().map((item) => {
            ...
            return ctx.cache.tryGet(link, async () => {
                ...
                const time = (() =>
                    $1('#version-toggle').text().trim() === '新加坡版' || $1('.after\\:content-dropdown-triangle').text().trim() === '新加坡版'
                        ? parseDate(JSON.parse($1("head script[type='application/ld+json']").eq(0).text())['@graph'][0]?.datePublished.replace(/(\d+\/\d+\/\d+) - (\d+:\d+)/, '$1 $2')) // SG
                        : parseDate(JSON.parse($1("head script[type='application/ld+json']").eq(1).text()).datePublished))(); // HK

    ...
    const items = await Promise.all(
        response.data.lists.map((item) => {
            ...
            return ctx.cache.tryGet(link, async () => {
                ...
                return {
                    title: item.title,
                    author: [{ name: $('.article-content__author').text().match('中央社') }, { name: data.publisher.name.match('轉角國際 udn Global') }, data.author].filter((e) => Boolean(e.name)),
                    description,
                    pubDate: timezone(parseDate(item.time.date, 'YYYY-MM-DD HH:mm'), +8),
                    category: [data.articleSection, vip ? $('.article-head li.breadcrumb__item:last > b').text() : $("meta[name='subsection']").attr('content'), ...(data.keywords ? data.keywords.split(',') : [])],
                    link,
                };
            });
        })
